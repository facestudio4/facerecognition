...
class Phase5EvaluatorPackTests(unittest.TestCase):
    def setUp(self):
    ...
    def test_build_bundle_outputs_summary_and_zip(self):
        builder = EvaluatorBundleBuilder(self.base_dir, self.db_path, host="127.0.0.1", port=8799)
        result = builder.build_bundle()
        self.assertTrue(os.path.exists(result["bundle_dir"]))
        self.assertTrue(os.path.exists(result["bundle_zip"]))
        self.assertTrue(os.path.exists(result["summary"]))
        self.assertTrue(os.path.exists(result["readme"]))
