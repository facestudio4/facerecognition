...
class DemoLaunchOrchestrator:
    def __init__(self, base_dir: str, db_path: str, host: str = "127.0.0.1", port: int = 8787):
        ...
        self.manifest_dir = os.path.join(base_dir, "demo_launcher")
    ...
    def prepare_demo(self):
        ...
        manifest = {
            ...
            "live_demo": demo_run,
            "suggested_commands": [
                "python facercognition.py phase6",
                "python facercognition.py judgesnapshot",
                "python facercognition.py judgedemo",
                "python facercognition.py evaluatorbundle",
            ],
        }
...
def launch_phase7_demo_launcher_gui(base_dir: str, db_path: str, parent=None, on_close_callback=None):
    ...
    win.title("Demo Launcher")
    win.geometry("980x700")
    win.configure(bg="#0b1320")

    tk.Label(win, text="Demo Launcher", font=("Segoe UI", 24, "bold"), fg="#ffb703", bg="#0b1320").pack(anchor="w", padx=14, pady=(12, 4))
    tk.Label(win, text="Prepare the full demo stack, generate manifest, and jump straight into judge mode", font=("Segoe UI", 11), fg="#b9d6ff", bg="#0b1320").pack(anchor="w", padx=14)

    status_var = tk.StringVar(value="Ready")
    tk.Label(win, textvariable=status_var, font=("Segoe UI", 10), fg="#7ad0ff", bg="#0b1320").pack(anchor="w", padx=14, pady=(6, 8))

    controls = tk.Frame(win, bg="#0b1320")
    controls.pack(fill="x", padx=14, pady=(0, 8))

    output = tk.Text(win, bg="#111b35", fg="#d7e2ff", font=("Consolas", 10), height=30)
